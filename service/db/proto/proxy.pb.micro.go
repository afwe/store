// Code generated by protoc-gen-micro. DO NOT EDIT.
// source: proxy.proto

package proto

import (
	fmt "fmt"
	proto "github.com/golang/protobuf/proto"
	math "math"
)

import (
	context "context"
	client "github.com/micro/go-micro/client"
	server "github.com/micro/go-micro/server"
)

// Reference imports to suppress errors if they are not otherwise used.
var _ = proto.Marshal
var _ = fmt.Errorf
var _ = math.Inf

// This is a compile-time assertion to ensure that this generated file
// is compatible with the proto package it is being compiled against.
// A compilation error at this line likely means your copy of the
// proto package needs to be updated.
const _ = proto.ProtoPackageIsVersion3 // please upgrade the proto package

// Reference imports to suppress errors if they are not otherwise used.
var _ context.Context
var _ client.Option
var _ server.Option

// Client API for DBService service

type DBService interface {
	ExecuteAction(ctx context.Context, in *ReqExec, opts ...client.CallOption) (*RespExec, error)
}

type dBService struct {
	c    client.Client
	name string
}

func NewDBService(name string, c client.Client) DBService {
	if c == nil {
		c = client.NewClient()
	}
	if len(name) == 0 {
		name = "proto"
	}
	return &dBService{
		c:    c,
		name: name,
	}
}

func (c *dBService) ExecuteAction(ctx context.Context, in *ReqExec, opts ...client.CallOption) (*RespExec, error) {
	req := c.c.NewRequest(c.name, "DBService.ExecuteAction", in)
	out := new(RespExec)
	err := c.c.Call(ctx, req, out, opts...)
	if err != nil {
		return nil, err
	}
	return out, nil
}

// Server API for DBService service

type DBServiceHandler interface {
	ExecuteAction(context.Context, *ReqExec, *RespExec) error
}

func RegisterDBServiceHandler(s server.Server, hdlr DBServiceHandler, opts ...server.HandlerOption) error {
	type dBService interface {
		ExecuteAction(ctx context.Context, in *ReqExec, out *RespExec) error
	}
	type DBService struct {
		dBService
	}
	h := &dBServiceHandler{hdlr}
	return s.Handle(s.NewHandler(&DBService{h}, opts...))
}

type dBServiceHandler struct {
	DBServiceHandler
}

func (h *dBServiceHandler) ExecuteAction(ctx context.Context, in *ReqExec, out *RespExec) error {
	return h.DBServiceHandler.ExecuteAction(ctx, in, out)
}
